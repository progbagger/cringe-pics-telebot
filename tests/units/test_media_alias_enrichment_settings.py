...
async def test_disabled_settings_ignore_ollama_configuration() -> None:
    settings = await load_media_alias_enrichment_settings(
        {
            "MEDIA_ALIAS_ENRICHMENT_ENABLED": "false",
            "OLLAMA_REQUEST_TIMEOUT_SECONDS": "not-a-number",
            "MEDIA_ALIAS_LLM_PROMPT": "Ignored prompt",
...
async def test_enabled_settings_are_normalized_and_keep_secrets_out_of_repr() -> None:
    settings = await load_media_alias_enrichment_settings(
        {
...
async def test_enabled_settings_require_ollama_values(missing: str) -> None:
    environ = {
    ...
    with pytest.raises(ValueError, match=missing):
        await load_media_alias_enrichment_settings(environ)
...
async def test_invalid_settings_are_rejected(*, name: str, value: str, message: str) -> None:
    environ = {
        "MEDIA_ALIAS_ENRICHMENT_ENABLED": "true",
        "OLLAMA_BASE_URL": "http://ollama:11434",
        "OLLAMA_MODEL": "gemma3:12b",
