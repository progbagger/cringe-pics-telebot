...
async def process_media_alias_enrichment_jobs(
    ...
    *,
    ...
    tasks = [asyncio.create_task(_process_job(job, ollama, settings, now=now, sleep=sleep)) for job in jobs]
    try:
...
async def run_media_alias_enrichment(
    ...
    *,
    ...
    while True:
        try:
            claimed = await process_media_alias_enrichment_jobs(ollama, settings, now=now, sleep=sleep)
...
async def _process_job(
    ...
    *,
    ...
    work = asyncio.create_task(_enrich_job(job, ollama, settings, now=now))
    heartbeat = asyncio.create_task(_heartbeat(job, settings, now=now, sleep=sleep))
    try:
        try:
            done, _ = await asyncio.wait({work, heartbeat}, return_when=asyncio.FIRST_COMPLETED)
            if work in done:
                result_class = await work
                _record_result(job, result_class, stopwatch)
            else:
                await heartbeat
                await _cancel_tasks((work,))
                _record_result(job, "lease_lost", stopwatch)
        except Exception as error:
            await _cancel_tasks((work,))
            try:
                result_class = await _handle_failure(job, error, settings, now=now)
                _record_result(job, result_class, stopwatch)
        ...
        result_class = await _wait_for_cleanup(_cancel_and_release(job, (work, heartbeat), now=now))
...
async def _enrich_job(
    ...
    *,
    ...
    prepare_stopwatch = Stopwatch.start()
    try:
        downloaded = await download_file(
            metadata.media.source_path,
        ...
        _emit(TimingMetric("media_alias_enrichment.media_prepare", prepare_stopwatch.elapsed_milliseconds()))
    assert settings.ollama_model is not None and settings.prompt is not None
    ollama_stopwatch = Stopwatch.start()
    try:
        values = await ollama.generate_aliases(image, model=settings.ollama_model, prompt=settings.prompt)
    finally:
        _emit(TimingMetric("media_alias_enrichment.ollama_request", ollama_stopwatch.elapsed_milliseconds()))
    aliases = parse_generated_media_aliases(values)
    return await _complete_job(job, aliases, settings, now=now)
...
async def _complete_job(
    ...
    *,
    ...
    async with transaction():
        ...
        timestamp = now()
        if media is None or not _owns_job(current_job, job, now=timestamp):
            return "lease_lost"
        metadata = await get_category_media_search_metadata(job.media_id)
    ...
    *,
    ...
    while True:
        ...
        async with transaction():
            ...
            timestamp = now()
            if not _owns_job(current_job, job, now=timestamp):
                return
